defmodule Mix.Tasks.Cpf.Gen do
  ...
  defp generate({_, _, invalids}) do
    {opt, _value} = List.first(invalids)
    error_exit("Invalid value for `#{opt}`")
  end

  defp format(opt) when opt in [nil, "standard"], do: {:ok, &CPF.format/1}
  defp format("digits"), do: {:ok, &to_string/1}
  defp format("integer"), do: {:ok, &(&1 |> CPF.to_integer() |> to_string())}
  defp format(option), do: {:error, {:format, option}}

  defp count(nil), do: {:ok, 1}
  defp count(n) when n > 0, do: {:ok, n}
  defp count(n), do: {:error, {:count, n}}
end
